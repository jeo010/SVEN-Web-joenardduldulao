<nav x-data="{ open: false }" class="">
    <div class="container mx-auto flex items-center">
        ...
        <button @click="open = !open" class="sm:hidden focus:outline-none" aria-label="Toggle menu">
            <svg x-show="!open" xmlns="http://www.w3.org/2000/svg" class="h-10 w-10" fill="none" viewBox="0 0 24 24"
                stroke="currentColor">
        ...
        <!-- Nav links -->
        <div 
            :class="open ? 'block bg-white p-4 rounded-md' : 'hidden'" 
            class="sm:flex sm:items-center sm:gap-10 sm:bg-transparent sm:p-0 text-lg"
        >
            <!-- Close (X) button inside dropdown -->
            <button 
                @click="open = false" 
                class="block md:hidden mb-4 focus:outline-none"
